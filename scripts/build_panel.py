...
build_panel.py
==============
Builds the monthly water main break panel for Kitchener, Ontario.

STEP 1 of the analysis pipeline. Must be run before build_final_dataset.py.

STUDY PERIOD: 1997-2025
  Although the break record nominally begins in 1985, a data audit found only
  10 break records across 1985-1996 versus 99 in 1997 alone, indicating that
  systematic logging began in 1997. Including the near-empty early years would
  introduce a spurious upward trend that any climate coefficient fitted over
  that window would partly absorb.

EXPOSURE (km in service):
  Primary panel uses the current inventory snapshot. Because retired pipes are
  absent from the snapshot, exposure is slightly understated in earlier years.
  A sensitivity panel estimates retired-pipe length from mean segment length
  by material; the resulting change in CI failure rate is under 3%, so the
  snapshot is retained as primary.
...
OUTPUTS:
  monthly_panel_primary.csv       material x month panel, 1044 rows
  monthly_panel_sensitivity.csv   same with retired-pipe exposure correction

COLUMNS PRODUCED:
  material, month_str, year, month_num, break_count, km_primary,
